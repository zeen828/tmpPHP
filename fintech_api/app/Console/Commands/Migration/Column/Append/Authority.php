...
class Authority extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'mg-column:append-authority {table? : Table in the database} {--y|yes : Allow execution}';
     ...
     * @var string
     */
    protected $description = 'Append a `authority` column for database table';
    ...
    {
        return base_path('resources' . DIRECTORY_SEPARATOR . 'stubs' . DIRECTORY_SEPARATOR . 'migrations' . DIRECTORY_SEPARATOR . 'column' . DIRECTORY_SEPARATOR . 'append' . DIRECTORY_SEPARATOR . 'Json.stub');
    }
    ...
    public function handle()
    {
        try {
            ...
            if ($this->option('yes') || $this->confirm('Do you want to save the creation `' . $this->getColumnName() . '` column on `' . $table . '` table?')) {
                ...
                $this->line('Library trait : App\\Libraries\\Traits\\Entity\\Column\\Authority');
