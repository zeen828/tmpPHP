...
class Source extends Command
{
     ...
     * @var string
     */
    protected $signature = 'config:add-receipt-source {name : The name is the entitie receipt source auth model class}';
     ...
     * @var string
     */
    protected $description = 'Insert receipt source basic configuration class';
    ...
    {
        try {
            $list = config('receipt.sourceables');

            if (! is_array($list)) {
                $this->error('Receipt source configuration insert failed.');
            } else {
                /* Input name */
                ...
                if (! isset($list[$code])) {
                    ...
                    $this->line('Receipt source \'' . $code . '\' is configured!', 'fg=red;bg=cyan');
                }

                $this->line('Library trait : App\\Libraries\\Traits\\Entity\\Receipt\\Auth');

                $this->info('Receipt source configuration insert successfully.');
            }
        } catch (\Throwable $e) {
