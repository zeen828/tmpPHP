...
class Feature extends Command
{
     ...
     * @var string
     */
    protected $signature = 'mg-column:append-feature {table? : Table in the database} {--y|yes : Allow execution}';
     ...
     * @var string
     */
    protected $description = 'Append a `feature` column for database table';
    ...
    public function handle()
    {
        try {
            ...
            if ($this->option('yes') || $this->confirm('Do you want to save the creation `' . $this->getColumnName() . '` column on `' . $table . '` table?')) {
                ...
                $this->line('Library trait : App\\Libraries\\Traits\\Entity\\Column\\Feature');
